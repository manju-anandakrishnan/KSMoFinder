...
class BilinearFeatureModule(nn.Module):
    def __init__(self, ip_dim1, ip_dim2, op_dim):
        ...
        self.bilinear2 = nn.Bilinear(ip_dim2, ip_dim2, op_dim)
        self.bilinear3 = nn.Bilinear(op_dim, op_dim, op_dim)
    
    
    def forward(self, kd_emb, s11_emb, k_st, s_st):
        out1 = self.bilinear1(kd_emb, s11_emb)
        out2 = self.bilinear2(k_st,s_st)
        combined = self.bilinear3(out1,out2)
        return combined
...
class BilinearDNNModel(nn.Module):
    ...
    def forward(self, kd_emb, s11_emb, k_st, s_st):
        combined_features = self.bilinear_module(kd_emb, s11_emb, k_st, s_st)
...
class KSFinder2:
    ...
    def __init__(self,training_data):        
        kd_emb_train = torch.tensor(training_data[0],dtype=torch.float32)
        s11_emb_train = torch.tensor(training_data[1],dtype=torch.float32)
        k_st_emb_train = torch.tensor(training_data[2],dtype=torch.float32)
        s_st_emb_train = torch.tensor(training_data[3],dtype=torch.float32)
        y_train = torch.tensor(training_data[4],dtype=torch.float32).view(-1,1)

        self.train_data_args = {'kd_emb':kd_emb_train, 's11_emb':s11_emb_train, 
                     'k_st_emb':k_st_emb_train, 's_st_emb':s_st_emb_train,
                     'y': y_train}
        
        self.dim_args = {'input_dim1':kd_emb_train.shape[1],
                     'input_dim2':k_st_emb_train.shape[1],
                     'output_dim':100}
    ...
    def train_model(self):
        ...
        for param_comb in param_combinations:
            ...
            k_emb_train, y_train = self.train_data_args['kd_emb'], self.train_data_args['y']
            ...
            for fold, (train_index, test_index) in enumerate(skf.split(k_emb_train,y_train)):
                ...
                try:
                    ...
                    for epoch in range(num_epochs):
                        # Forward pass
                        tr_outputs = model(X_fold_train_cuda['kd_emb'],
                        ...
                        with torch.no_grad():
                            model.eval()
                            val_outputs = model(X_fold_test_cuda['kd_emb'],
    ...
    def train_best_model(self,best_model_params):
        ...
        for _ in range(best_model_params.get('epoch')):
            outputs = model(train_data_args_cuda['kd_emb'],
...
if __name__ == '__main__':
    ...
    model_path = constants.MODEL_STRUCT_SEQ

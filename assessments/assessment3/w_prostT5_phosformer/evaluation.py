...
def evaluate_model(model,testing_data):   
    ...
    kd_emb_test = torch.tensor(testing_data[3],dtype=torch.float32).to(device)
    s11_emb_test = torch.tensor(testing_data[4],dtype=torch.float32).to(device)
    ...
    model.eval()
    with torch.no_grad():        
        outputs = model(k_emb_test,s_emb_test,m_emb_test,
                            kd_emb_test,s11_emb_test,k_st_emb_test,s_st_emb_test)

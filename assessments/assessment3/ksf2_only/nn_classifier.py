...
class BilinearFeatureModule(nn.Module):
    def __init__(self, ip_dim1, op_dim):
    ...
    def forward(self, k_emb, s_emb, m_emb):
        out1 = self.bilinear1(k_emb, s_emb)
        out2 = self.bilinear2(k_emb, m_emb)
        combined1 = self.bilinear3(out1,out2)
        return combined1
...
class BilinearDNNModel(nn.Module):
    def __init__(self, ip_dim1, op_dim, hidden_sizes, output_size):
        super(BilinearDNNModel, self).__init__()
        self.bilinear_module = BilinearFeatureModule(ip_dim1, op_dim)
        self.dnn = MultiLayerDNN(hidden_sizes, output_size)
    
    def forward(self, k_emb, s_emb, m_emb):
        combined_features = self.bilinear_module(k_emb, s_emb, m_emb)
...
class KSFinder2:
    ...
    def train_model(self):
        ...
        for param_comb in param_combinations:
            ...
            for fold, (train_index, test_index) in enumerate(skf.split(k_emb_train,y_train)):
                ...
                try:
                    # Create the model
                    model = BilinearDNNModel(input_dim1, output_dim, param_comb.get('hidden_sizes'), self.dnn_output_size)
    ...
    def train_best_model(self,best_model_params):
        input_dim1 = self.dim_args.get('input_dim1')
        output_dim = self.dim_args.get('output_dim')
        model = BilinearDNNModel(input_dim1, output_dim, \
...
if __name__ == '__main__':
    ...
    model_path = constants.MODEL_FUNC
